...
$signemail = textinput($_POST['signEmail']);
$signpassword1 = textinput(password_hash($_POST['signpassword1'], PASSWORD_BCRYPT));
$password2 = textinput(password_hash($_POST['password2'], PASSWORD_BCRYPT));
$hash = $mysqli->escape_string( md5( rand(0,1000) ) );
    if(empty($_POST["firstname"]) || empty($_POST["lastname"]) || empty($_POST["studentid"]) || empty($_POST["signEmail"]) || empty($_POST["signpassword1"]) ||empty($_POST["password2"])){
		...
		$result = $mysqli->query("SELECT * FROM users WHERE email='$signemail' OR id='$student_id'") or die($mysqli->error());
		...
		if ( $result->num_rows > 0 ) {
			...
			if($_POST['signpassword1']!= $_POST['password2']){
	...
	// active is 0 by DEFAULT (no need to include it here)
				...
				$sql = "INSERT INTO users (first_name, last_name, id, email, password, hash) " 
						. "VALUES ('$first_name','$last_name','$student_id','$signemail','$signpassword1', '$hash')";
				...
				if ( $mysqli->query($sql) ){
					...
					$_SESSION['message'] .='<p><label class="text-danger">Confirmation link has been sent to '.$signemail.', please verify your account by clicking on the link in the message!</label></p>';
					...
					http://localhost/Assignment3/verify.php?email='.$signemail.'&hash='.$hash;  
